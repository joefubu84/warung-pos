...
## 🍳 5.1 Paparan Dapur (Kitchen Display) & Kebenaran Realtime RLS

Jalankan skrip ini dalam **Supabase SQL Editor** untuk membolehkan skrin Dapur (`/kitchen`) dan senarai pesanan membaca pesanan secara realtime tanpa disekat oleh RLS (terutama apabila staf masuk melalui mod kecemasan):

```sql
-- 1. Berikan kebenaran SELECT dan UPDATE kepada anon untuk jadual orders & order_items
...
-- 2. Pastikan Realtime aktif untuk jadual orders dan order_items
DO $$
BEGIN
  IF NOT EXISTS (
