...
function LoyaltyPage() {
  ...
  const getTierBadge = (tier: string) => {
    if (tier === 'Platinum') return <span className="bg-purple-100 text-purple-800 border border-purple-200 px-2.5 py-1 rounded-full text-xs font-bold font-mono">💎 Platinum VIP</span>;
    if (tier === 'Gold') return <span className="bg-amber-100 text-amber-800 border border-amber-300 px-2.5 py-1 rounded-full text-xs font-bold font-mono">🥇 Gold VIP</span>;
    if (tier === 'Silver') return <span className="bg-slate-100 text-slate-700 border border-slate-300 px-2.5 py-1 rounded-full text-xs font-bold font-mono">🥈 Silver Member</span>;
    return <span className="bg-emerald-100 text-emerald-800 border border-emerald-300 px-2.5 py-1 rounded-full text-xs font-bold font-mono">🥉 Bronze Member</span>;
  };

  const emojiPresets = ['💎', '🎁', '🥤', '☕', '🍗', '🍱', '🍔', '💵', '🌟', '🏷️', '🎉', '🔥'];

  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 p-4 md:p-8 font-sans pb-24">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-slate-200/90 p-6 rounded-3xl shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Customer Loyalty & VIP Rewards</h1>
            </div>
          ...
          <div className="flex items-center gap-2">
            <Button 
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-5 py-2.5 rounded-xl shadow-xs flex items-center gap-2 text-xs font-mono"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-mono">
          <div className="bg-white border border-slate-200/90 p-4 rounded-2xl shadow-xs">
            <span className="text-xs text-slate-500 block">Total VIP Members</span>
            <span className="text-2xl font-black text-emerald-700">{members.length}</span>
          </div>

          <div className="bg-white border border-slate-200/90 p-4 rounded-2xl shadow-xs">
            <span className="text-xs text-slate-500 block">Total Active Points</span>
            <span className="text-2xl font-black text-amber-700">{totalPointsIssued} pts</span>
          </div>

          <div className="bg-white border border-slate-200/90 p-4 rounded-2xl shadow-xs">
            <span className="text-xs text-slate-500 block">Gold VIPs (500+ pts)</span>
            <span className="text-2xl font-black text-sky-700">{goldCount}</span>
          </div>

          <div className="bg-white border border-slate-200/90 p-4 rounded-2xl shadow-xs">
            <span className="text-xs text-slate-500 block">Platinum VIPs (1k+ pts)</span>
            <span className="text-2xl font-black text-purple-700">{platinumCount}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                  <User className="w-5 h-5 text-emerald-600" /> Member Directory
                </h2>
                <p className="text-xs text-slate-500 font-mono">Search by phone number, customer name, or VIP tier</p>
              </div>
              ...
              <div className="relative w-full sm:w-64">
                ...
                <Input
                  ...
                  className="bg-slate-50 border-slate-200 text-slate-900 pl-9 text-xs rounded-xl font-mono focus:bg-white"
                />
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredMembers.map((member) => (
                    <tr key={member.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 text-slate-700">{member.phone}</td>
                      ...
                      <td className="py-3 text-right flex items-center justify-end gap-1.5">
                        <Button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-[11px] px-2.5 py-1 rounded-lg gap-1 shadow-2xs font-mono"
                        >
                        ...
                        <Button
                          ...
                          className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-[11px] px-3 py-1 rounded-lg gap-1 shadow-2xs"
                        >
                  ...
                  {filteredMembers.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-12 text-center text-slate-500">
                        ...
                        <p className="font-bold text-slate-800 text-sm">Tiada Ahli Berdaftar Buat Masa Ini</p>
          ...
          <div className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                  <Gift className="w-5 h-5 text-amber-500" /> Rewards Catalog
                </h2>
                <p className="text-xs text-slate-500 font-mono">Available reward vouchers ({rewardsCatalog.length})</p>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <Button
                  ...
                  className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs px-2.5 py-1.5 rounded-lg flex items-center gap-1 shadow-xs font-mono"
                >
                ...
                {rewardsCatalog.length > 0 && (
                  <Button
                    ...
                    className="border-slate-200 bg-white text-rose-600 hover:bg-rose-50 hover:text-rose-700 text-xs px-2 py-1.5 rounded-lg"
                  >
            ...
            <div className="space-y-3 font-mono max-h-[520px] overflow-y-auto pr-1">
              {rewardsCatalog.map((reward) => (
                <div key={reward.id} className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-2 hover:border-amber-300 transition-all">
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex items-start gap-2.5 flex-1 min-w-0">
                      <span className="text-2xl shrink-0 mt-0.5">{reward.emoji}</span>
                      <div className="min-w-0 flex-1">
                        <h4 className="font-bold text-slate-900 text-xs truncate">{reward.title}</h4>
                        <p className="text-[10px] text-slate-500 line-clamp-2 mt-0.5">{reward.description}</p>
                    ...
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        <Button
                          ...
                          className="h-6 w-6 p-0 text-slate-500 hover:text-slate-900 hover:bg-slate-200 rounded-md"
                          title="Edit Ganjaran"
                        >
                        ...
                        <Button
                          ...
                          className="h-6 w-6 p-0 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-md"
              ...
              {rewardsCatalog.length === 0 && (
                <div className="py-10 text-center text-slate-500 space-y-3">
                  <Gift className="w-8 h-8 mx-auto text-slate-400 opacity-50" />
                  <p className="text-xs text-slate-700 font-bold">Tiada Baucar Ganjaran</p>
                  ...
                  <Button
                    ...
                    className="border-slate-300 text-amber-700 hover:bg-amber-50 text-xs gap-1.5 mx-auto font-mono"
                  >
      ...
      <Dialog open={isRewardModalOpen} onOpenChange={setIsRewardModalOpen}>
        <DialogContent className="bg-white text-slate-900 border-slate-200 max-w-md font-sans rounded-3xl shadow-xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg font-bold text-amber-600 font-mono">
          ...
          <form onSubmit={handleSaveReward} className="space-y-4 my-2">
            ...
            <div className="space-y-1.5">
              <Label className="text-slate-700 text-xs font-bold">Pilih Ikon / Emoji</Label>
                ...
                {emojiPresets.map((emoji) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-lg text-lg flex items-center justify-center transition-all ${
                      rewEmoji === emoji ? 'bg-amber-100 border border-amber-400 scale-110' : 'hover:bg-slate-200'
                    }`}
                  >
                    {emoji}
                  </button>
            ...
            <div className="space-y-2">
              <Label className="text-slate-700 text-xs font-bold">Nama Baucar / Ganjaran *</Label>
              <Input
                required
                placeholder="e.g. Diskaun RM 5.00 Ahli"
                value={rewTitle}
                onChange={(e) => setRewTitle(e.target.value)}
                className="bg-slate-50 border-slate-200 text-slate-900 text-xs font-mono focus:bg-white"
              />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label className="text-slate-700 text-xs font-bold">Mata Diperlukan (pts) *</Label>
                <Input
                  required
                  type="number"
                  min="1"
              ...
              <div className="space-y-2">
                <Label className="text-slate-700 text-xs font-bold">Jenis Ganjaran *</Label>
                <select
                  value={rewType}
                  onChange={(e) => setRewType(e.target.value as any)}
                  className="w-full h-9 rounded-md bg-slate-50 border border-slate-200 text-slate-900 text-xs px-3 font-mono focus:outline-none focus:ring-1 focus:ring-amber-500"
                >
            ...
            <div className="space-y-2">
              <Label className="text-slate-700 text-xs font-bold">Penerangan / Syarat Ganjaran</Label>
              <Input
            ...
            <DialogFooter className="pt-2">
              <Button type="button" variant="outline" onClick={() => setIsRewardModalOpen(false)} className="border-slate-300 text-slate-700 text-xs">
                Batal
              </Button>
      ...
      <Dialog open={isRegisterOpen} onOpenChange={setIsRegisterOpen}>
        <DialogContent className="bg-white text-slate-900 border-slate-200 max-w-md font-sans rounded-3xl shadow-xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg font-bold text-emerald-700 font-mono">
          ...
          <form onSubmit={handleSaveMember} className="space-y-4 my-2">
            <div className="space-y-2">
              <Label className="text-slate-700 text-xs font-bold">Customer Phone Number *</Label>
              <Input
                required
                placeholder="e.g. 60172221784"
                value={regPhone}
                onChange={(e) => setRegPhone(e.target.value)}
                className="bg-slate-50 border-slate-200 text-slate-900 font-mono focus:bg-white"
              />
            </div>

            <div className="space-y-2">
              <Label className="text-slate-700 text-xs font-bold">Customer Name</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="text-slate-700 text-xs font-bold">Add Bonus Points</Label>
              <Input
                type="number"
                value={regAddPoints}
            ...
            <DialogFooter className="pt-2">
              <Button type="button" variant="outline" onClick={() => setIsRegisterOpen(false)} className="border-slate-300 text-slate-700">
                Cancel
              </Button>
      ...
      <Dialog open={isRedeemOpen} onOpenChange={setIsRedeemOpen}>
        <DialogContent className="bg-white text-slate-900 border-slate-200 max-w-md font-sans rounded-3xl shadow-xl">
          ...
          {selectedMember && (
            <div className="space-y-3 my-2 font-mono">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex justify-between items-center text-xs">
                <span>Member: <strong className="text-slate-900">{selectedMember.name}</strong></span>
                <span className="text-amber-700 font-black">{selectedMember.points} pts available</span>
              </div>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {rewardsCatalog.map((reward) => {
                  ...
                  return (
                    <div key={reward.id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span className="text-lg">{reward.emoji}</span>
                        <div>
                          <div className="font-bold text-slate-900 text-xs">{reward.title}</div>
                          <span className="text-[10px] text-amber-700">{reward.pointsRequired} pts required</span>
                        </div>
                      </div>
                      <Button
                        size="sm"
          ...
          <DialogFooter className="pt-2">
            <Button variant="outline" onClick={() => setIsRedeemOpen(false)} className="border-slate-300 text-slate-700">
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
