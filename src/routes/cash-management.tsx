...
export function CashManagementPage() {
  ...
  const [openStaffNameInput, setOpenStaffNameInput] = useState<string>(() => {
    const active = getActiveStaffUser();
    return active?.name || 'Admin Warung J&J';
  });
  ...
  const handleOpenRegister = async (e: React.FormEvent) => {
    ...
    const effectiveStaffName = openStaffNameInput.trim() || staffName || 'Admin Warung J&J';
    setStaffName(effectiveStaffName);
    ...
    try {
      ...
      try {
        const { error } = await supabase
          ...
          .insert({
            ...
            opened_by_name: effectiveStaffName
          });

        if (!error) {
          inserted = true;
        } else if (error.code !== 'PGRST205' && !error.message?.includes('daily_cash')) {
          // If column opened_by_name doesn't exist, retry without it
        ...
        if (insertErr?.code !== 'PGRST205' && !insertErr?.message?.includes('daily_cash')) {
          console.warn('daily_cash insert notice:', insertErr);
        }
      }
      ...
      toast.success(`Kaunter berjaya dibuka oleh ${effectiveStaffName} dengan baki awal RM ${balance.toFixed(2)}`);
...
_Report generated by ${dailyCash?.opened_by_name || staffName} at ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}_`;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white border border-slate-200/90 p-6 rounded-3xl shadow-xs flex flex-col justify-between">
                ...
                <div className="pt-4 border-t border-slate-100 mt-4 flex justify-between items-center">
                  <span className="text-xs text-slate-500">Opened by <strong className="text-slate-800 font-semibold">{dailyCash?.opened_by_name || staffName}</strong></span>
                  <span className="text-xs text-slate-500 font-mono">
                    {dailyCash.opened_at || dailyCash.created_at ? new Date(dailyCash.opened_at || dailyCash.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                  </span>
                </div>
              </div>
      ...
      <Dialog open={isOpeningModal} onOpenChange={setIsOpeningModal}>
        <DialogContent className="bg-white text-slate-900 border-slate-200 max-w-md rounded-3xl shadow-xl">
          ...
          <div className="space-y-4 py-3 font-mono">
            <div>
              <label className="text-xs text-slate-600 uppercase font-bold block mb-1.5">
                Nama Staf / Juruwang Bertugas <span className="text-rose-500">*</span>
              </label>
              <Input
                type="text"
                value={openStaffNameInput}
                onChange={(e) => setOpenStaffNameInput(e.target.value)}
                placeholder="cth: Admin Warung J&J / Siti"
                className="bg-slate-50 border-slate-200 text-slate-900 font-bold rounded-xl text-sm"
              />
              <p className="text-[11px] text-slate-400 font-sans mt-1">Nama ini akan dicetak di atas resit jualan dan rekod audit kaunter.</p>
            </div>
        ...
        <DialogContent className="bg-white text-slate-900 border-slate-200 max-w-md font-mono rounded-3xl shadow-xl">
          ...
          <div id="printable-sales-report" className="bg-white text-black p-6 rounded-2xl font-mono text-xs space-y-3 shadow-sm border border-slate-200">
            <div className="text-center space-y-1 pb-3 border-b-2 border-dashed border-black">
              ...
              <p className="text-[10px] text-gray-700 font-bold">Staff / Juruwang: {dailyCash?.opened_by_name || staffName}</p>
            </div>
